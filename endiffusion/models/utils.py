...
def remove_mean_with_mask(x, node_mask, fix_size=None):
    #print(x.size(), node_mask.size())
    # x.cpu()
    # node_mask.cpu()
    masked_max_abs_value = (x * (~node_mask)).abs().sum()
    # if masked_max_abs_value > 1e-5:
    #     print(x, node_mask)
    assert masked_max_abs_value < 1e-5, f'Error {masked_max_abs_value} too high'
    if fix_size is None:
        fix_size = x.size(1)
    N = node_mask[:, :fix_size].sum(1, keepdims=True)
