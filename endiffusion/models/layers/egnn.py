...
class EGNN(nn.Module):
    ...
    def forward(self, h, x, edges, edge_attr=None, node_mask=None, edge_mask=None):
        ...
        """
        we remove the edge_attr here to make the things clean. 
        """
        edge_mask = edge_mask.view(edge_mask.size(0)*edge_mask.size(1),1)

        h = self.embedding(h)
        for i in range(0, self.n_layers):
            h, x, _ = self._modules["gcl_%d" % i](h, edges, x, node_mask=node_mask, edge_mask=edge_mask)
