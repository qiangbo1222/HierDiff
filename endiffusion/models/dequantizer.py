...
from models.layers.egnn import EGNN
...
class EGNN_output_h(nn.Module):
    def __init__(self, in_node_nf, out_node_nf, hidden_nf=64,
        ...
        # self, in_node_nf, in_edge_nf, hidden_nf, act_fn="silu", n_layers=3, attention=False,
    ...
    def forward(self, x, h, node_mask, edge_mask):
        # print("x type is node_mask",x.size())
        # print("h type is h", h.size())
        # print("node mask is edge mask",node_mask.size())
        # print("edge mask is x",edge_mask.size())

        bs, n_nodes, dims = x.shape
        assert self.in_node_nf == h.size(2)

        node_mask = node_mask.view(bs * n_nodes, 1)
        edges = self.get_adj_matrix(n_nodes, bs, x.device)
...
class UniformDequantizer(nn.Module):
    ...
    def forward(self, tensor, node_mask, edge_mask, x):
        category, integer = tensor['categorical'].float(), tensor['integer'].float()
        zeros = torch.zeros(integer.size(0), device=integer.device)
        # print(category.size(),category)
...
class VariationalDequantizer(nn.Module):
    ...
    def sample_qu_xh(self, node_mask, edge_mask, x, h):
        ...
        assert (mu * (~node_mask)).sum() < 1e-5 and \
               (log_sigma * (~node_mask)).sum() < 1e-5, \
    ...
    def forward(self, tensor, node_mask, edge_mask, x):
        # print ("h should be ", tensor.size())
        # print ("x should be", x.size())
        categorical, integer = tensor['categorical'], tensor['integer']
...
class ArgmaxAndVariationalDequantizer(VariationalDequantizer):
    ...
    def reverse(self, tensor):
        ...
        return {'categorical': categorical, 'integer': integer}
#currently used uniform_dequantizer to conduct a fair comparision on the 