...
def mol_equal(mol1, mol2):
    mol1_c = Chem.RemoveHs(mol1)
    mol2_c = Chem.RemoveHs(mol2)
    for atom in mol1_c.GetAtoms():
        atom.SetAtomMapNum(0)
    for atom in mol2_c.GetAtoms():
        atom.SetAtomMapNum(0)
    '''
    if mol1_c.GetNumAtoms() != mol2_c.GetNumAtoms():
        return False
    if mol1_c.GetNumBonds() != mol2_c.GetNumBonds():
        return False
    for atom1,atom2 in zip(mol1_c.GetAtoms(), mol2_c.GetAtoms()):
        if not atom_equal(atom1, atom2):
            return False
    for bond1,bond2 in zip(mol1_c.GetBonds(), mol2_c.GetBonds()):
        if bond1.IsInRing() and bond2.IsInRing():
            if not ring_bond_equal(bond1, bond2):
                return False
        else:
            if not bond1.GetBondType() == bond2.GetBondType():
                return False
    '''
    if Chem.MolToSmiles(mol1_c) != Chem.MolToSmiles(mol2_c):
        return False
    #check mapping
    for atom2 in mol2.GetAtoms():
        ...
        if atom2.GetAtomMapNum() != 0:
            for atom1 in mol1.GetAtoms():
                if atom1.GetAtomMapNum() == atom2.GetAtomMapNum() and atom_equal(atom1, atom2):
                    find_map = True
                    break
            if not find_map:
                return False
