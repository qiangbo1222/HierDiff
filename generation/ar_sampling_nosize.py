...
def prepare_data(feat, position, adj_matrix, node_types={}, vocab_size=780):
...
def update_trees(model, model_refine,trees, vocab, beam_size=5, device=torch.device('cpu')):
    """
    ...
    if len(trees) > 0:
        ...
        edges_result, node_predict, adj_matrix = model.sample_AR(data_batch)#, x_perturb
        #print(edges_result)
...
def sample_trees_from_blur(jts, model, model_refine, vocab, cfg, device, context_norm=1):
    ...
    with torch.no_grad():
        ...
        for i, jt in enumerate(jts):
            ...
            nodes = [MolTreeNode_blur(jt['h'][i], jt['x'][i], 0) for i in range(jt['h'].shape[0])]
        ...
        while not q.empty():
            ...
            if tree.end:
                ...
                q = remove_queue_dup(q, tree.index_, 0, pool)
...
if __name__ == '__main__':
    ...
    dataset_list = random_split(whole_dataset, [train_size, valid_size, test_size])
    train_dataset, valid_dataset, test_dataset = dataset_list[0], dataset_list[1], dataset_list[2]
    dataset_list = random_split(train_dataset, [train_size-100, 100])
    data = [{'h': d['feat'][:, :], 'x': d['position']} for d in dataset_list[1]]
    '''
