...
class JTNNDecoder(nn.Module):
    ...
    def decode(self, mol_vec, prob_decode):
        ...
        root = MolTreeNode(self.vocab.get_smiles(root_wid), pos=np.zeros(3))
        root.wid = root_wid
        root.idx = 0
        ...
        for step in range(MAX_DECODE_LEN):
            ...
            if not backtrack: #Forward: Predict next clique
                ...
                for wid in sort_wid[:5]:
                    ...
                    node_y = MolTreeNode(self.vocab.get_smiles(wid), pos=np.zeros(3))
                ...
                if next_wid is None:
                    ...
                    node_y = MolTreeNode(self.vocab.get_smiles(next_wid), pos=np.zeros(3))
                    node_y.wid = next_wid
                    node_y.idx = step + 1
                    node_y.neighbors.append(node_x)
...
def can_assemble(node_x, node_y=None):
    neis = node_x.neighbors
    if node_y is not None and node_y not in neis:
        neis.append(node_y)
    ...
    #if len(cands) == 0:
    #    print(f'{node_x.smiles} cannot be assembled with {node_y.smiles}')
    return len(cands) > 0
