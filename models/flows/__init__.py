...
class Flow(torch.nn.Module):
    ...
    def forward(self, x, func='adaptive', step_size=None, node_mask=None, edge_mask=None, context=None):
        ldj = x.new_zeros(x.shape[0])
        reg_term = x.new_zeros(x.shape[0])
        
        actnorm = self.transformations[0]
        x, delta_logp, delta_reg_term = actnorm(x, node_mask=node_mask, edge_mask=edge_mask, context=context)
        ldj = ldj + delta_logp
        reg_term = reg_term + delta_reg_term

        for transform in self.transformations[1:]:
            x, delta_logp, delta_reg_term = transform(x, func, step_size=step_size, node_mask=node_mask, edge_mask=edge_mask, context=context)
            ldj = ldj + delta_logp
            reg_term = reg_term + delta_reg_term
        '''
        for transform in self.transformations:
            x, delta_logp, delta_reg_term = transform(x, node_mask=node_mask, edge_mask=edge_mask, context=context)
            ldj = ldj + delta_logp
            reg_term = reg_term + delta_reg_term
        '''
